...
ADC_Regular_Channel_Config (ADC_SFRmap* ADCx, uint32_t Channel, uint32_t Rank)
{
	...
	else if (Rank < 8)
	{
		...
		tmpmask = ADC_SCANSQ1_SSQ4 << (8 * (Rank - 4));
	...
	else if (Rank < 12)
	{
		...
		tmpmask = ADC_SCANSQ2_SSQ8 << (8 * (Rank - 8));
	...
	else
	{
		...
		tmpmask = ADC_SCANSQ3_SSQ12 << (8 * (Rank - 12));
		ADCx->SCANSQ3 = SFR_Config (ADCx->SCANSQ3,
							  ~tmpmask,
							  tmpreg);
	}
...
ADC_HPConversion_DMA_Enable (ADC_SFRmap* ADCx, uint8_t HPChannel, FunctionalState NewState)
{
    ...
    /*------------- 配置ADCx_CTL0寄存器HPyDMAEN位 -------------*/
    tmpreg = NewState << (HPChannel + ADC_CTL0_HP0DMAEN_POS);
    tmask = 1 << (HPChannel + ADC_CTL0_HP0DMAEN_POS);
