...
uint8_t CAN_Get_Point_Of_RAM_Mailbox(CAN_SFRmap* CANx)
{
    ...
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_CAN_ALL_PERIPH(CANx));
    tmpreg = (CANx->RCR & CAN_RCR_RAMMB0) >> CNA_RCR_RAMMB0_POS;
...
CAN_Turn_Cailbration_Enable(CAN_SFRmap* CANx, FunctionalState NewState)
{
    ...
    /*------------- 配置CANx_CTLR寄存器TRIEN位 -------------*/
    CANx->CTLR = SFR_Config(CANx->CTLR, ~CAN_CTLR_TRIEN, NewState<<CAN_CTLR_TRIEN_POS);
}
...
CAN_Receive_Message_Configuration_From_Register (CAN_SFRmap* CANx, CAN_MessageTypeDef* canInitStruct)
{
    ...
    /* 获取RXDATA0-3，存储区域第一个数据 */
    tmpreg[0] = CANx->RXDATA0;//RXDATA0
    tmpreg[1] = CANx->RXDATA1;//RXDATA1
    tmpreg[2] = CANx->RXDATA2;//RXDATA2
    tmpreg[3] = CANx->RXDATA3;//RXDATA3

    /* CAN帧格式 */
	canInitStruct->m_FrameFormat = (tmpreg[0] & CAN_INFR_IDE) >> CAN_INFR_IDE_POS;

	/* CAN远程发送请求 */
	canInitStruct->m_RemoteTransmit = (tmpreg[0] & CAN_INFR_RTR) >> CAN_INFR_RTR_POS;

    /* 数据长度 */
    canInitStruct->m_DataLength = (tmpreg[0] & CAN_INFR_DLC) >> CAN_INFR_DLC0_POS;

    if (canInitStruct->m_FrameFormat != CAN_FRAME_FORMAT_EFF)
    { /* 标准格式SFF */
        /* 标准帧ID */
        canInitStruct->m_StandardID = (tmpreg[1] & CAN_TX0R_SFF_ID) >> CAN_TX0R_ID18_POS;

        /*判断是否为远程帧 */
        if(canInitStruct->m_RemoteTransmit != CAN_DATA_FRAME)
        {
        	return ;
        }
    ...
    {	/* 扩展帧格式EFF */
        /* 扩展帧ID */
        canInitStruct->m_ExtendedID = (tmpreg[1] & CAN_TX0R_EFF_ID) >> CAN_TX0R_ID0_POS;
